...
$sql = "SELECT oc.ID, uv.UsuarioID, uv.Rut, uv.Nombres, uv.Apellidos, uv.Genero, oc.OfertaCapacitacionID, oa.NombreOTEC, oa.RutEmpresaOTEC, oa.NombreCurso, oa.NombreContacto, oa.CorreoContacto, oa.TelefonoContacto, oa.Horarios, oa.Lugar, oa.EstudiosRequeridos, oa.DocumentacionRequerida, oa.GruposObjetivos FROM sg_omil_usuariosvecinales AS uv JOIN sg_omil_seleccionados_capacitacion AS oc ON uv.UsuarioID = oc.UsuarioID JOIN sg_omil_ofertascapacitacion AS oa ON oc.OfertaCapacitacionID = oa.OfertaCapacitacionID";
...
<head>
    ...
    <title>Administración de Seleccionados Capacitacion</title>
...
<div id="wrapper">
    ...
    <h2>Administración de Seleccionados Capacitacion</h2>
    <div class="row">
        <div class="col-md-12">
            <!-- Advanced Tables -->
            <div class="panel panel-default">
                <div class="panel-heading">
                    Lista de Seleccionados de Capacitacion a Usuairios Vecinales
                </div>
                        ...
                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                            <thead>
                                <tr>
                                    ...
                                    <th>Apellidos</th>
                                    <th>Genero</th>
                                    <th>ID Oferta de Capacitacion</th>
                                    <th>Rut de Empresa OTEC</th>
                            ...
                            <tbody>
                                ...
                                if ($result->num_rows > 0) {
                                    while ($row = $result->fetch_assoc()) {
                                        ...
                                        echo "<td>{$row['OfertaCapacitacionID']}</td>";
                                        echo "<td>{$row['RutEmpresaOTEC']}</td>";
                                        echo "<td>{$row['NombreOTEC']}</td>";
                                        ...
                                        if ($rol === "administrador"):
                                            echo "<td>";
                                            echo "<br>";
                                            echo "<a class='delete-btn' href='eliminar_seleccionado_capacitacion.php?id={$row['ID']}'>Eliminar</a>";
                                            endif;
                                            echo "<br>";
                                            echo "<br>";
                                            echo "<a class='edit-btn' href='notificarUsuario.php?usuarioId={$row['UsuarioID']}&ofertaId={$row['OfertaCapacitacionID']}'>Notificar</a>";
                                            echo "</td>";
                                        echo "</tr>";
                                    }
                                }
